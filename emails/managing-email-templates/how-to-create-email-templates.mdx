---
title: "How to Create Email Templates"
description: "An email template is a pre-defined email layout, that may already include content like images or text. Rather than create a new email from scratch eac..."
---

An email template is a pre-defined email layout, that may already include content like images or text. Rather than create a new email from scratch each time, you can use a template as a base. Templates are also handy for making sure email designs follow brand guidelines.Topics Covered,

[**How to Create an Email Template**](https://support.salesmate.io/hc/en-us/articles/6309996481177-How-to-Create-Email-Templates#h_01H5SECMF1KTV3JVZTTP9AWAV5)[**Text Editor Options**](https://support.salesmate.io/hc/en-us/articles/6309996481177-How-to-Create-Email-Templates#h_01H5SFKY1TVF118BCQ4PMDYWS7)

To create Campaign Templates please check this article**\-**[**Article Link**](https://support.salesmate.io/hc/en-us/articles/19847146942361 "/hc/en-us/articles/19847146942361")

### How to Create an Email Template

To create email templates:

Navigate to the**More Icon**from the left Menu barClick on**Email Templates**Head over to**New Template**to create a template.

![mceclip0.png](./images/mceclip0-2.png)

Here add the following details to start creating a New Template,**Template Name:**Add a name to the template for using it while you send the emails**Folder:**Manage your email templates based on your requirement**Subject:**Add the purpose of the email**Body:**Add email content and use attributes to personalize the emails

![mceclip1.png](./images/mceclip1-2.png)

### Text Editor Options

Use a different text editor option to customize the email content**More Text:**With this option, you can customize the content by changing**Font Style, Font Size, Font Family, Text color, and Background color**. You can also**Underline the Content**,**Use Strikethrough**, and also**Clear Formatting**from here.

![mceclip2.png](./images/mceclip2.png)

**More Paragraph:**With this option, you can align the content to the Left, Right, or Center. Furthermore, you can also use the Order or Unorder List for alignment.

![mceclip3.png](./images/mceclip3.png)

**Insert Link:**This option allows you to add any links to your content.

![mceclip4.png](./images/mceclip4.png)

**Insert Image:**This option allows you to add any image or gif to personalize your content by simply adding the links or using the Media Manager

![mceclip5.png](./images/mceclip5.png)

**Insert table:**You can use this option to add content in table format.

![mceclip6.png](./images/mceclip6.png)

**Meeting Scheduler:**With this option, you can add Meeting links to your content.

![mceclip11.png](./images/mceclip11.png)

**Insert Attributes:**Insert attributes to personalize your content

![mceclip10.png](./images/mceclip10.png)

**Code View:**View email in the HTML editor

![mceclip9.png](./images/mceclip9.png)

Click on**Save**to save the template
