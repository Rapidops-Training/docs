...
- **Exit Rules:** Define conditions to stop the capture step if the user fails to provide input (e.g., after a number of attempts). Write rules in natural language (e.g., “Exit if user says ‘I prefer not to share my email"_._). You can set up to **10 rules**. Exit Rules are only available if **Re-prompt Automatically** is enabled.
