...
- **Please ensure that the name & the address on the LOA match the one on the Invoice/Customer Service Record.**(LOA will be provided by the team according to different country and type of number)**Please send an email to support@salesmate.io with the documents and details.
...
- ** Submit your Porting request **to **support@salesmate.io.
